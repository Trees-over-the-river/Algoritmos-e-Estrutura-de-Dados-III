...
import sorts.intercalation.substitute.SubstitutionHeapNode;
 ...
 */
public class SortedFileHeap<T extends Register<T>> extends BinaryArchive<T> {

    private static final int BLOCK_SIZE = 4096; // 4KB
    private static final int NUMBER_OF_BRANCHES = 3; // Number of branches for the sort algorithm
    ...
    private int distribute() throws IOException {
        ...
        MinHeap<SubstitutionHeapNode<T>> heap = new MinHeap<>(this.originalNumberOfRegistersPerBlock);
        ...
        while(this.database.getPosition() < this.database.length()) {
            ...
            if(heap.size() == this.originalNumberOfRegistersPerBlock) {
                SubstitutionHeapNode<T> tmp;

                if(this.comparator.compare(obj, heap.peek().getItem()) >= 0) {
                    tmp = heap.substitute(new SubstitutionHeapNode<T>(obj, heap.peek().getWeight(), comparator));
                } else {
                    tmp = heap.substitute(new SubstitutionHeapNode<T>(obj, heap.peek().getWeight() + 1, comparator));
                ...
                if(heap.size() == 0) 
                    heap.insert(new SubstitutionHeapNode<T>(obj, 0, comparator));
                else
                    heap.insert(new SubstitutionHeapNode<T>(obj, heap.peek().getWeight(), comparator));
            }
        }

        while(heap.size() != 0) {
            SubstitutionHeapNode<T> tmp = heap.remove();
            j = tmp.getWeight() % NUMBER_OF_BRANCHES;
            numberOfBlocks = tmp.getWeight() + 1;
